...
class Domain
{
    ...
    public function exists(): bool
    {
        if (is_null($this->ex_f)) {
            $st = Database::connection()->prepare('SELECT `id` FROM `' . Database::tablePrefix() . 'domains` WHERE ' . $this->sqlCondition());
    ...
    public function save(): void
    {
        ...
        if ($this->exists()) {
            try {
                $st = $db->prepare('UPDATE `' . Database::tablePrefix() . 'domains` SET `active` = ?, `description` = ?, `updated_time` = FROM_UNIXTIME(?) WHERE `id` = ?');
            ...
            try {
                ...
                $st = $db->prepare('INSERT INTO `' . Database::tablePrefix() . 'domains` (`fqdn`, `active`' . $sql1 . ', `created_time`, `updated_time`) VALUES (?, ?' . $sql2 . ', FROM_UNIXTIME(?), FROM_UNIXTIME(?))');
                $idx = 0;
    ...
    private function fetchData(): void
    {
        ...
        $st = Database::connection()->prepare('SELECT `id`, `fqdn`, `active`, `description`, `created_time`, `updated_time` FROM `' . Database::tablePrefix() . 'domains` WHERE ' . $this->sqlCondition());
        $this->sqlBindValue($st, 1);
        $st->execute();
        $res = $st->fetch(PDO::FETCH_NUM);
        if (!$res) {
