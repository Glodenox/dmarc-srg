...
class DomainList
{
    ...
    {
        $list = [];
        $st = Database::connection()->query('SELECT `id`, `fqdn`, `active`, `description`, `created_time`, `updated_time` FROM `' . Database::tablePrefix() . 'domains`');
    ...
    {
        $st = Database::connection()->query('SELECT COUNT(*) FROM `' . Database::tablePrefix() . 'domains`', PDO::FETCH_NUM);
